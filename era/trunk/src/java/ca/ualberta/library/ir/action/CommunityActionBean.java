 ...
 * Project: ir
 * $Id: CommunityActionBean.java 5606 2012-10-10 16:45:09Z pcharoen $
 */
...
public class CommunityActionBean extends BaseActionBean {
	...
	public Resolution save() {
		try {
			...
			props.setOwnerId(object.getOwnerId().getValue());
			...
			context.getMessages().add(
				new LocalizableMessage("community.modifySuccess", trimTitle(object.getTitle().get(0))));
		...
		try {
			...
			if (resp.getResults().getNumFound() > 0) {
				...
				context.getValidationErrors().addGlobalError(
					new LocalizableError("community.communityNotEmpty", trimTitle(object.getTitle().get(0))));
			}
			...
			context.getMessages().add(
				new LocalizableMessage("community.deleteSuccess", trimTitle(object.getTitle().get(0))));
			return new ForwardResolution(uiPath + "/protected/editCommunityMessage.jsp");
		} catch (Exception e) {
