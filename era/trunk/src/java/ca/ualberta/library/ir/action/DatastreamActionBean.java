 ...
 * Project: ir
 * $Id: DatastreamActionBean.java 5606 2012-10-10 16:45:09Z pcharoen $
 */
...
public class DatastreamActionBean extends BaseActionBean {
	...
	public Resolution get() {
		try {
			...
			// craete get method
			String url = fedoraServerUrl + "/" + fedoraContext + "/get/" + pid + "/" + dsId;
			...
			try {
				...
				if (dsId.startsWith(DS.toString()) || dsId.equals(LICENSE.toString())) {
					...
					org.fcrepo.server.types.gen.Datastream dstm = services.getDatastream(pid, dsId);
					try {
				...
				if (dsId.equals(THUMBNAIL.toString())) {
					...
					return forwardErrorMessage("Could not get datastream content!");
				}
			}
		} catch (Exception e) {
