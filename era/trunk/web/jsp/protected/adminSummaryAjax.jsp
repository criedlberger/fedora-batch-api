...
<div id="admin">
	<h2>
		<a href="#" class="adminButton" onclick="toggleAdminSummary(); return false;"><fmt:message key="admin.sidebar.name" />
		</a>
	</h2>
	<ul id="adminMenu">
		...
		<c:forEach begin="1" end="${count}" var="i" varStatus="status">
			...
			<c:if test="${empty roles || enabled}">
				<li><a href="${ctx}${url}"><fmt:message key="${name}.${i}.name" />
				</a> <c:choose>
						...
						<c:when test="${roles == '/admin/index/builder'}">
							<fmt:formatDate value="${actionBean.index.startTime}" pattern="MMM d h:mm a" /> (${actionBean.index.hours}:${actionBean.index.minutes}:${actionBean.index.seconds})</span>
				</li>
				</c:when>
				<c:otherwise>
