	...
	<c:forEach items="${actionBean.inputFields}" var="fld" varStatus="status">
		<li style="clear: both;${fld.inputType.value == 'hidden' ? ' display: none;' : ''}" class="${fld.indent ? 'indented' : ''}">
			...
			<c:when test="${fld.inputType.value == 'dropdown'}">
				<c:choose>
				<c:when test="${actionBean.context.user.group.id == ADMIN_GROUP}">
					<c:set var="valuePairsName" value="${fld.inputType.valuePairsName}-admin" />
					<c:if test="${empty actionBean.valuePairsMap[valuePairsName]}">
						<c:set var="valuePairsName" value="${fld.inputType.valuePairsName}" />
					</c:if>
				</c:when>
				<c:otherwise>
					<c:set var="valuePairsName" value="${fld.inputType.valuePairsName}" />
				...
				<div id="field.${status.index}00" class="menus field">
					...
					<c:forEach items="${actionBean.valuePairsMap[valuePairsName].pairs}" var="pair">
