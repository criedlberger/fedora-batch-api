<%--
	$Id: readingPages.tag 5427 2012-07-12 20:30:12Z pcharoen $
...
<%@ attribute name="path" type="java.lang.String" required="true" description="The reading action path. Ex: /public/read/uuid:.../DS1" %>
...
<c:if test="${pages > 1}">
	...
	<c:if test="${currentPage > 1}">
		<span><a href="${path}/${start - rows}">&laquo; <fmt:message key="pages.previous" /></a>
		<c:if test="${startPage > 1}"><a href="${path}/1">1</a></c:if>
	...
	<c:forEach begin="${startPage}" end="${endPage}" var="page">
		...
		<c:if test="${page != currentPage}">
			<span><a href="${path}/${page}">${page}</a></span>
		</c:if>
	</c:forEach>
	<span>
	<c:if test="${endPage < pages}"> ... </c:if>
	<c:if test="${endPage < numFound}">
		<a href="${path}/${numFound}">${numFound}</a>
	</c:if>
	</span>
	<c:if test="${currentPage < pages}">
		<span><a href="${path}/${start + rows}"><fmt:message key="pages.next" /> &raquo;</a></span>
	</c:if> 
</c:if>
