<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/2.0.0/web/jsp/public/browseCommunity.jsp $
   $Id: browseCommunity.jsp 5615 2012-10-16 18:20:47Z pcharoen $
			...
			<c:forEach items="${actionBean.results}" var="result" varStatus="status">
				<div class="a_community">
					<div class="title">
	                    ...
	                    <a id="${fn:toUpperCase(fn:substring(result['dc.title'][0], 0, 1))}" href="${ctx}/public/view/community/${result['PID']}" title="${title}">${result['dc.title'][0]}</a>
                    </div>
                   	<div class="toolbar">
	                    <div id="subcription.${status.index}" class="subscription"></div>
	                    <script type="text/javascript">
	                    	getSubscriptionInfo('subcription.${status.index}', '${result["PID"]}', <%= SubscriptionType.COMMUNITY.getValue() %>);
	                    </script>
	                    <a href="${ctx}/public/feed/communityitems/${feedType}/${result['PID']}" target="_blank" class="rssbutton">
	                    	<fmt:message key="toolbar.feed.label" />
	                    </a>
	                    <link rel="alternate" type="application/rss+xml" title="${result['dc.title']}" href="${ctx}/public/feed/communityitems/${feedType}/${result["PID"]}" /> 
                    </div>
                </div>
			</c:forEach>
