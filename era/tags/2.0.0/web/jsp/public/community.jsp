<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/2.0.0/web/jsp/public/community.jsp $
   $Id: community.jsp 5615 2012-10-16 18:20:47Z pcharoen $
	...
	<stripes:layout-component name="contents">
		<div class="full_box">
			...
			<div class="a_collection">
				<div class="title">
					<a id="${fn:toUpperCase(fn:substring(result['dc.title'][0], 0, 1))}" href="${ctx}/public/view/collection/${result['PID']}" class="result_title">${result['dc.title'][0]}</a>
				</div>
				<div class="toolbar">
					<div id="subcription.${status.index}" class="subscription"></div>
					<script type="text/javascript">
						getSubscriptionInfo('subcription.${status.index}', '${result["PID"]}', <%= SubscriptionType.COLLECTION.getValue() %>);
					</script>
					<div>
						<a href="${ctx}/public/feed/collectionitems/${feedType}/${result['PID']}" target="_blank" class="rssbutton">
							<fmt:message key="toolbar.feed.label" />
						</a>
						<link rel="alternate" type="application/rss+xml" title="${result['dc.title'][0]}" href="${ctx}/public/feed/collectionitems/${feedType}/${result['PID']}" /> 
					</div>
				</div>
			</div>
			</c:forEach>
