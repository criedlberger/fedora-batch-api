<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/2.0.0/web/jsp/protected/approvalList.jsp $
   $Id: approvalList.jsp 5615 2012-10-16 18:20:47Z pcharoen $
			...
			<c:forEach items="${actionBean.results}" var="result" varStatus="status">
				<li class="record">
					...
					<div class="itemlist_actions">
						<c:choose>
						<c:when test="${result['era.workflowState'] == 'Submit'}">
							<stripes:label for="WorkflowState.${result['era.workflowState']}" />${' '}<fmt:message key="by" />${' '}<br />
						...
						<c:if test="${result['era.workflowState'] == 'Submit'}">
						<a href="${ctx}/action/approval/review/${result['PID']}" class="edit_button" onclick="bigWaiting(); return true;"><fmt:message key="button.takeTask" /></a>
						</c:if>
						<a href="${ctx}/public/view/item/${result['PID']}/workflow" class="add_button auto_width" id="mltImg"><fmt:message key="item.view.transactions" /></a>
						</security:secure>
					</div>
				</li>
			</c:forEach>
