<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/2.0.0/web/jsp/protected/deletedItemList.jsp $
   $Id: deletedItemList.jsp 5615 2012-10-16 18:20:47Z pcharoen $
			...
			<c:forEach items="${actionBean.results}" var="result" varStatus="status">
				<li class="record">
					...
					<div class="itemlist_actions">
						...
						<ir:user username="${result['era.userId']}" var="usr">
							<a href="mailto:${usr.email}">${usr.firstName}${' '}${usr.lastName}</a><br />
						</ir:user>
						<fmt:formatDate pattern="${actionBean.dateFormat}" value="${result['fo.lastModifiedDate_dt']}" />
						<%-- 
						<security:secure roles="/admin/deleted,/item/delete">
						<a href="${ctx}/action/item/undeleted/${result['PID']}" class="delete_button" onclick="bigWaiting(); return true;"><fmt:message key="button.undeleted" /></a>
						</security:secure>
						--%>
					</div>
				</li>
