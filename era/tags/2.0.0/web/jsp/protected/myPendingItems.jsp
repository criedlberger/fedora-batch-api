<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/2.0.0/web/jsp/protected/myPendingItems.jsp $
   $Id: myPendingItems.jsp 5615 2012-10-16 18:20:47Z pcharoen $
			...
			<c:forEach items="${actionBean.results}" var="result" varStatus="status">
				<li class="record">
					...
					<div class="itemlist_actions">
						...
						<c:otherwise>
							<stripes:label for="Item.WorkflowState.${result['era.workflowState']}" />
						</c:otherwise>
						</c:choose>
						...
						<c:if test="${result['era.workflowState'] == 'Reject'}">
						<a href="${ctx}/action/submit/editrejected/${result['era.formName']}/${result['PID']}" class="edit_button" onclick="bigWaiting(); return true;"><fmt:message key="button.edit" /></a>
						</c:if>
						</security:secure>
					</div>
				</li>
