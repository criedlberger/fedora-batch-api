 ...
 * Project: ir
 * $Id: SubmitActionBean.java 5615 2012-10-16 18:20:47Z pcharoen $
 */
...
import ca.ualberta.library.ir.model.inputform.Pair;
...
public class SubmitActionBean extends BaseActionBean {
	...
	@Before(on = { "edit", "editsaved", "editrejected", "editembargoed", "email" }, stages = { LifecycleStage.EventHandling })
	...
	private void initMetadataInputForm() throws Exception {
		...
		// enable/disable thesis form depending on thesis-graduation-dates date range
		if (inputForm.getName().equals(Form.Name.THESIS.toString())) {
			Date now = new Date();
			List<Pair> pairs = valuePairsMap.get("thesis-graduation-dates").getPairs();
			for (Pair pair : pairs) {
				if (now.compareTo(pair.getDateRange().getDateStart()) >= 0
					&& now.compareTo(pair.getDateRange().getDateEnd()) < 0) {
	...
	public Resolution email() {
		try {
			if (item.getProperties().getWorkflowState().equals(WorkflowState.Submit.toString())) {
				sendSubmitMail();
				return forwardMessage("submit.email.message");
			} else {
		...
		Thread thread = new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					log.error("Could not send email! (" + item.getProperties().getPid() + ")", e);
				}
			}
		});
		thread.start();
