 ...
 * Project: ir
 * $Id: ApprovalActionBean.java 5430 2012-07-12 22:30:19Z pcharoen $
 */
					...
					try {
						...
						if (!user.getUsername().equals(item.getProperties().getUserId())) {
							if (item.getProperties().getUserId() != null) {
								services.purgeRelationship(item.getProperties().getPid(), Definitions.USER_ID.getURI(),
									item.getProperties().getUserId(), true, null);
							}
						...
						log.error("Could not update object to review item! (" + item.getProperties().getPid() + ")", e);
					}
				}
			});
