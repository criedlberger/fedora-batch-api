 ...
 * Project: ir
 * $Id: CollectionActionBean.java 5430 2012-07-12 22:30:19Z pcharoen $
 */
...
public class CollectionActionBean extends BaseActionBean {
	...
	private void initProperties(String pid) throws SolrServerException, FedoraAPIException {
		...
		collection.setSort(sortRels != null ? sortRels[0].getObject() : null);
		RelationshipTuple[] metaRels = services.getRelationships(pid, Definitions.META_DESCRIPTION.getURI());
		collection.setMetaDescription(metaRels != null ? BooleanUtils.toBoolean(metaRels[0].getObject()) : false);
		RelationshipTuple[] formNameSERRels = services.getRelationships(pid, Definitions.FORM_NAME.getURI());
		collection.setFormName(formNameSERRels != null ? formNameSERRels[0].getObject() : null);
		RelationshipTuple[] proquestUploadRels = services.getRelationships(pid, Definitions.PROQUEST_UPLOAD.getURI());
		collection.setProquestUpload(proquestUploadRels != null ? BooleanUtils.toBoolean(proquestUploadRels[0]
			.getObject()) : false);
	}
	...
	public Resolution save() {
		try {
			...
			context.getMessages()
				.add(new LocalizableMessage("collection.modifySuccess", trimTitle(object.getTitle(0))));
	...
	private void saveObject() throws Exception {
		...
		properties.setLabel(collection.getTitle());
		properties.setContentModel(Collection.CONTENT_MODEL);
		properties.setOwnerId(object.getOwnerId());
	...
	private void getCollectionDetails() throws SolrServerException, FedoraAPIException, ServiceException {
		collection.setId(object.getPid());
		collection.setState(object.getState());
		...
		collection.setModifiedDate(object.getMDate());
		...
		try {
			...
			if (results.getNumFound() > 0) {
				context.getValidationErrors().addGlobalError(
					new LocalizableError("collection.collectionNotEmpty", trimTitle(object.getTitle(0))));
			}
			...
			context.getMessages()
				.add(new LocalizableMessage("collection.deleteSuccess", trimTitle(object.getTitle(0))));
			return new ForwardResolution(uiPath + "/protected/editCollectionMessage.jsp");
	...
	private void getCollectionDetails(ObjectFields object) throws FedoraAPIException, SolrServerException {
		...
		collection.setId(object.getPid());
		collection.setState(object.getState());
		collection.setTitle(object.getLabel());
		DcDocument dc = services.getDublinCore(object.getPid());
		// log.debug(dc.toString());
		DublinCore dublinCore = new DublinCore(dc);
		collection.setDescription(dublinCore.getFields().get(4).getValues().get(0));
		collection.setModifiedDate(object.getMDate());
		collection.setCreatedDate(object.getCDate());
		collection.setOwnerId(object.getOwnerId());

		// get item relationships
		memberOfCommunities = services.getMemberOfCommunities(object.getPid());
