<!--
   $HeadURL: https://code.library.ualberta.ca/svn/repos/era/tags/1.3.12/era-1.3/web/jsp/protected/deposit.jsp $
   $Id: deposit.jsp 5427 2012-07-12 20:30:12Z pcharoen $
...
<stripes:layout-render name="/jsp/layout/standard.jsp" title="Deposit" active="3">
	...
	<stripes:layout-component name="contents">
		<div class="full_box">
			...
			<div class="subheader">
				<p><fmt:message key="deposit.subheader" /></p>
			</div>
			<c:if test="${actionBean.item.properties.workflowState == 'Reject'}">
			<div class="errors">
				<h2>
				<fmt:message key="deposit.itemRejected">
				<fmt:param>
					<c:choose>
				...
				<ul>
					<c:if test="${not empty actionBean.item.comments}">
						<li>${actionBean.item.comments}</p></li> 
					</c:if>
				</ul>
			</div>
			</c:if>
			...
			<stripes:form id="itemForm" action="/action/deposit" focus="owner">
				...
				<security:secure roles="/admin/depositor">
							...
							<li> 
								...
								<div class="menus">
									<stripes:select name="itemComs" id="item.coms" onchange="addCommunity(this); return false;">
										<option value=""><fmt:message key="deposit.select" /></option>
										<stripes:options-collection collection="${context.allCommunities}" label="title" value="id"/>
									</stripes:select>
								</div>
					...
					<fieldset class="save_item">
						<p class="regular"><fmt:message key="deposit.item.save" /></p>
					</fieldset>
					...
					<stripes:submit name="submit" class="deposit_button" onclick="bigWaiting(); return true;"><fmt:message key="button.deposit" /></stripes:submit>
					<stripes:submit name="save" class="save_button" style="margin-left: 0; height: 2em; width: 4em;" onclick="bigWaiting(); return true;"><fmt:message key="button.save" /></stripes:submit>
					</security:secure>
					<!-- TODO: check form change before cancel -->
					<a class="cancel_link" href="${ctx}/action/${event == 'edit' ? 'myaccount/savedItems' : 'deposit'}"><fmt:message key="button.cancel" /></a>
			</stripes:form>
			</div>
		</div>
	</stripes:layout-component>
