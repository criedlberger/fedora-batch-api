 ...
 * Project: ir-2.0
 * $Id: CheckoutActionBean.java 5430 2012-07-12 22:30:19Z pcharoen $
 */
			...
			for (String pid : cart) {
				...
				for (Datastream dstm : dstms) {
					...
					if (dstm.getID().startsWith(DatastreamID.DS.toString())) {
						try {
							if (getDownloadDatastreamContent(n++, pid, dstm.getID(), tmpDir, itemPattern,
								results.getTitle(0))) {
								dlPids.add(pid);
				...
				if (i > 0) {
					for (Datastream dstm : dstms) {
						if (dstm.getID().equals(DatastreamID.LICENSE.toString())) {
							try {
								getDownloadDatastreamContent(n++, pid, dstm.getID(), tmpDir, itemPattern,
									results.getTitle(0));
							} catch (FedoraAPIException e) {
